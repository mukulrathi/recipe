...
use himiklab\thumbnail\EasyThumbnailImage as BaseEasyThumbnailImage;

class EasyThumbnailImage  extends BaseEasyThumbnailImage
{
     ...
     */
    public static function thumbnail($filename, $width, $height, $mode = self::THUMBNAIL_OUTBOUND, $quality = 50, $checkRemFileMode = self::CHECK_REM_MODE_NONE)
    {
        return Image::getImagine()->open(self::thumbnailFile($filename, $width, $height, $mode, $quality, $checkRemFileMode));
    }
     ...
     * @throws \yii\base\InvalidParamException
     */
    public static function thumbnailFile($filename, $width, $height, $mode = self::THUMBNAIL_OUTBOUND, $quality = 50, $checkRemFileMode = self::CHECK_REM_MODE_NONE)
    {
        ...
        if (preg_match('/^https?:\/\//', $filename)) {
            ...
            $filename = FileHelper::normalizePath(Yii::getAlias(self::$uploadsAlias . $filename));
            if (!is_file($filename)) {
                $filename = self::$uploadsAlias . self::$defaultImage;
            }
            if (!is_file($filename)) {
     ...
     * @throws \yii\base\InvalidParamException
     */
    public static function thumbnailFileUrl($filename, $width, $height, $mode = self::THUMBNAIL_OUTBOUND, $quality = 50, $checkRemFileMode = self::CHECK_REM_MODE_NONE)
    {
     ...
     * @return string
     */
    public static function thumbnailImg($filename, $width, $height, $mode = self::THUMBNAIL_OUTBOUND, $options = [], $quality = 50, $checkRemFileMode = self::CHECK_REM_MODE_NONE)
    {
        try {
